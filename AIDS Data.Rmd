...
- Possible error data point with value of 288 `priorzdv` (max)
...
# Could also try CVing to maximise concordance index
...
preds # Some vector of risk predictions

# stime <- aids$time

# sevent <- aids$censor

weight <- runif(length(preds), min = 0, max = 1)

comppairs <- 10 #??????

# strat <- whatever the strat is? not sure
